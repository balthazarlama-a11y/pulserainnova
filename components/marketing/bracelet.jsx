const Bracelet = ({ size = 340, tone = "lavender", floating = true, pulse = true }) => {
  ...
  return (
    ...
    >
      {/* Halo aurora detrás del bracelet */}
      <div
        className="absolute inset-[-22%] pointer-events-none blur-3xl"
        style={{ background: `radial-gradient(circle, ${tones.accent}55, transparent 60%)` }}
      />
      <div
        className="absolute inset-[-12%] pointer-events-none blur-2xl animate-breathe-slow"
        style={{ background: `radial-gradient(circle, ${tones.accent}33, transparent 65%)` }}
      />

      <svg viewBox="0 0 400 340" width="100%" height="100%" className="relative z-[2]">
        <defs>
        ...
        {/* Correa izquierda */}
        <path d="M 90 70 Q 40 110 50 180 Q 60 250 130 270"
              stroke={`url(#strap-${tone})`} strokeWidth="42" strokeLinecap="round" fill="none" />
        <path d="M 90 70 Q 40 110 50 180 Q 60 250 130 270"
              stroke={`url(#strapShade-${tone})`} strokeWidth="42" strokeLinecap="round" fill="none" opacity="0.6" />
        ...
        {/* Correa derecha */}
        <path d="M 280 80 Q 350 120 350 190 Q 348 250 290 275"
              stroke={`url(#strap-${tone})`} strokeWidth="42" strokeLinecap="round" fill="none" />
        <path d="M 280 80 Q 350 120 350 190 Q 348 250 290 275"
              stroke={`url(#strapShade-${tone})`} strokeWidth="42" strokeLinecap="round" fill="none" opacity="0.6" />
        ...
        <g transform="translate(130,70)">
          <rect x="0" y="0" width="160" height="200" rx="36"
                fill={`url(#module-${tone})`} stroke="rgba(255,255,255,0.1)" strokeWidth="1" />
          ...
          <g transform="translate(80,100)" fill="none" stroke={tones.accent} strokeLinecap="round">
            <path d="M -44 0 L -28 0 L -22 -18 L -14 22 L -6 -10 L 2 6 L 10 0 L 44 0"
                  strokeWidth="2" opacity="0.95" />
            <text x="0" y="-30" textAnchor="middle" fill={tones.accent} stroke="none"
                  style={{ font: "600 18px Inter", letterSpacing: 0.5 }}>72</text>
            <text x="0" y="-14" textAnchor="middle" fill={tones.accent} stroke="none" opacity="0.6"
                  style={{ font: "400 7px Inter", letterSpacing: 1 }}>BPM</text>
            <text x="0" y="34" textAnchor="middle" fill={tones.accent} stroke="none" opacity="0.9"
                  style={{ font: "500 8px Inter", letterSpacing: 1.5 }}>CALMA</text>
