// Kids view — animated character, breathing exercise, mini-game

// Friendly blob character — reacts to stress/mood
const CalmChar = ({ mood = 'calm', size = 220, blinking = true }) => {
  const colors = {
...
// Breathing circle — expand/contract with phases
        ...
        {done ? (
          <div style={{ animation: 'heroTextIn 0.6s' }}>
            ...
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 16, marginBottom: 32 }}>
              Completaste 5 respiraciones. ¿Cómo te sientes ahora?
            </p>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ textAlign: 'center' }}>
                  <div style={{ fontFamily: 'Fraunces, serif', fontSize: 32, fontWeight: 500, color: '#2a1848', marginBottom: 4 }}>
                    {phaseLabel}
                  </div>
                  <div style={{ fontSize: 40, fontWeight: 600, color: '#2a1848', letterSpacing: -2 }}>
                    {cfg[phase] - phaseSec}
                  </div>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginTop: 14 }}>
              {Array.from({ length: totalCycles }).map((_, i) => (
                <div key={i} style={{
                  ...
                  background: i < cycle ? '#B8A4FF' : 'rgba(255,255,255,0.12)',
...
// Mini-game — pop bubbles in rhythm
const BubblePop = ({ onClose }) => {
  ...
  const pop = (id) => {
    setBubbles(bs => bs.filter(x => x.id !== id));
    setScore(s => s + 10);
  };
  ...
  return (
    <div style={{
      position: 'fixed', inset: 0, zIndex: 100,
      background: 'linear-gradient(180deg, #2a1849 0%, #0f0825 100%)',
      overflow: 'hidden'
    }}>
      <button onClick={onClose} style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: '#E9D6FF', fontSize: 14, fontWeight: 600
        }}>✨ {score}</div>
      </div>
      ...
      {done ? (
        <div style={{
          ...
          animation: 'heroTextIn 0.6s'
        }}>
          <div style={{ fontSize: 80, marginBottom: 12 }}>🎉</div>
          <h2 style={{ fontFamily: 'Fraunces, serif', fontSize: 42, fontWeight: 500, margin: '0 0 8px' }}>
            ¡Increíble!
          </h2>
          <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.7)', marginBottom: 24 }}>
            Lograste <strong style={{ color: '#B8A4FF' }}>{score} puntos</strong>
          </p>
          <Button size="lg" onClick={onClose}>Volver <IconArrowRight size={14}/></Button>
        </div>
      ) : (
        <>
          {bubbles.map(b => (
            <div key={b.id}
                 ...
                 style={{
                   ...
                   cursor: 'pointer',
                   animation: `bubbleRise ${b.dur}s linear forwards`
                 }}/>
          ))}
          <div style={{
...
const KidsView = ({ stress, onBack }) => {
  const state = stressState(stress);
  const mood = state.key;
  const [activity, setActivity] = React.useState(null); // null | 'breath' | 'bubble'
  const greeting = stress <= 30 ? '¡Qué bien te ves hoy!' :
                   stress <= 55 ? '¿Cómo vas, Sofía?' :
                   stress <= 75 ? '¿Necesitas un momento?' :
                                  'Estoy aquí contigo';
    ...
    <div style={{
      ...
      <header style={{
        ...
        <button onClick={onBack} style={{
          ...
          <IconArrowLeft size={14}/> Papá/mamá
        </button>
        <div style={{
          ...
          <span style={{ fontSize: 13 }}>Sofía · 8 años</span>
        </div>
      </header>
      ...
      <main style={{
        ...
        <h1 style={{
          ...
          fontWeight: 500, margin: '0 auto 12px', maxWidth: 640, lineHeight: 1.1, letterSpacing: '-0.02em'
        }}>
          {greeting}
        </h1>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[1,2,3,4,5,6,7].map(d => (
              <div key={d} style={{
                ...
                background: d <= 5 ? '#F5D06F' : 'rgba(255,255,255,0.1)',
                boxShadow: d <= 5 ? '0 0 6px #F5D06F88' : 'none'
              }}/>
            ))}
          </div>
          <div style={{ fontSize: 13, color: 'var(--ink-muted)' }}>
            <strong style={{ color: '#F5D06F' }}>5 días</strong> cuidándote 🌟
          </div>
        </div>
      </main>
...
const ActivityCard = ({ accent, icon, title, sub, recommended, onClick }) => (
  ...
  onMouseEnter={e => { e.currentTarget.style.transform='translateY(-3px)'; e.currentTarget.style.boxShadow=`0 12px 30px -10px ${accent}55`;}}
  ...
  >
    {recommended && (
      <div style={{
        ...
        background: accent, color: '#0A0A1A', fontSize: 10, fontWeight: 700, letterSpacing: 0.5
      }}>RECOMENDADO</div>
    )}
    <div style={{
...
Object.assign(window, { KidsView, CalmChar });
