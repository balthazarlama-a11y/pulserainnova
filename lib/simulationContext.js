...
export function SimulationProvider({ children }) {
  ...
  // Simula un episodio de ansiedad en vivo:
  // Arranca a las 13:00 del miércoles (ansiedad subiendo) a 3x,
  // así el pico de 80 en ansiedad llega ~20s después y el usuario ve la escalada completa.
  const triggerStressEvent = useCallback(() => {
    setActive(true);
    setSpeed(3);           // 3 min de simulación por segundo → escalada visible
    setCurrentDay(2);      // Miércoles — pelea con amiga, pico ansiedad 80
    setCurrentHour(13);
    setCurrentMinute(0);
    setFiredEvents(new Set());
    setLatestEvent(null);
    setPlaying(true);
    setPanelOpen(false);   // Panel cerrado para ver el dashboard en vivo
  }, []);
