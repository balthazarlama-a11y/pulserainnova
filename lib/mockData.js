...
// Pseudo-random determinístico: misma salida en SSR y en cliente para evitar
// hydration mismatches en HistoryClient.
const seeded = (i, salt) => {
  const x = Math.sin(i * 9.71 + salt * 31.4) * 10000;
...
export function getSessionHistory() {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    const avgStress = Math.max(10, Math.min(85, Math.round(30 + Math.sin(i * 0.7) * 20 + seeded(i, 1) * 10)));
    ...
    sessions.push({
      ...
      avgStress,
      avgBpm,
      peakStress: Math.min(95, avgStress + Math.floor(seeded(i, 2) * 20)),
      exercisesCompleted: Math.floor(seeded(i, 3) * 4),
      duration: `${Math.floor(6 + seeded(i, 4) * 8)}h ${Math.floor(seeded(i, 5) * 60)}m`,
    });
  }
  return sessions;
}
